...
function decodeJwtPayload(token: string): any {
  try {
    ...
    console.error('[TokenValidation] Erro ao decodificar JWT:', error);
    return null;
  }
}
...
function isTokenExpired(token: string): boolean {
  try {
    ...
    if (!payload || !payload.exp) {
      console.warn('[TokenValidation] Token sem campo exp');
    ...
    if (isExpired) {
      console.log('[TokenValidation] Token expirado:', {
        expiration: new Date(expirationTime).toISOString(),
        now: new Date(now).toISOString(),
      });
    } else {
      const hoursUntilExpiry = Math.round((expirationTime - now) / (1000 * 60 * 60));
      console.log('[TokenValidation] Token válido, expira em:', hoursUntilExpiry, 'horas');
    }

    return isExpired;
  } catch (error) {
    console.error('[TokenValidation] Erro ao verificar expiração:', error);
...
export async function validateStoredToken(): Promise<boolean> {
  try {
    console.log('[TokenValidation] Iniciando validação do token...');

    const storedUser = await AsyncStorage.getItem('@app:user');

    if (!storedUser) {
      console.log('[TokenValidation] Nenhum usuário armazenado');
      return false;
    }
    ...
    try {
      ...
      console.error('[TokenValidation] Erro ao parsear dados do usuário:', parseError);
    ...
    if (!userData.token) {
      console.log('[TokenValidation] Token não existe nos dados do usuário');
      return false;
    }
    ...
    if (tokenParts.length !== 3) {
      console.log('[TokenValidation] Token com estrutura inválida');
      await AsyncStorage.removeItem('@app:user');
      return false;
    }
    ...
    if (isTokenExpired(userData.token)) {
      console.log('[TokenValidation] Token expirado - limpando dados');
      await AsyncStorage.removeItem('@app:user');
      return false;
    }

    console.log('[TokenValidation] Token válido localmente');
    return true;
  } catch (error) {
    console.error('[TokenValidation] Erro na validação:', error);
    await AsyncStorage.removeItem('@app:user');
    return false;
  }
}
...
export async function validateTokenWithBackend(): Promise<boolean> {
  try {
    console.log('[TokenValidation] Validando token com backend...');
    ...
    if (response.status === 401) {
      console.log('[TokenValidation] Backend retornou 401 - token inválido');
      await AsyncStorage.removeItem('@app:user');
      return false;
    }

    // Qualquer outro status (200, 404, etc) significa que o token é aceito
    console.log('[TokenValidation] Token validado pelo backend, status:', response.status);
    return true;
  } catch (error: any) {
    // Se for erro de rede, não invalida o token (pode ser problema de conexão)
    if (!error.response) {
      console.warn('[TokenValidation] Erro de rede ao validar token, mantendo sessão');
    ...
    console.warn('[TokenValidation] Erro ao validar com backend:', error.message);
    return true;
  }
}
