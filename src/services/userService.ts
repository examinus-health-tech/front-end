...
export async function getUserPersonalData(): Promise<UserPersonalDataResponseDTO | null> {
  try {
    ...
    if (__DEV__) console.log('[USER_SERVICE] Buscando perfil completo do usuário');
    ...
    if (__DEV__) console.log('[USER_SERVICE] Status final:', response.status);

    // Se for 404, retornar null
    if (response.status === 404) {
      if (__DEV__) console.log('[USER_SERVICE] Usuário ainda não tem dados cadastrados (404)');
      return null;
    }
    ...
    if (__DEV__) console.log('[USER_SERVICE] Perfil recuperado com sucesso');

    return userData as UserPersonalDataResponseDTO;
  } catch (error: any) {
    if (__DEV__) console.error('[USER_SERVICE] Erro ao buscar perfil completo:', error.message);
    if (__DEV__) console.error('[USER_SERVICE] Status do erro:', error.response?.status);
    throw error;
  }
}
...
export async function updateUserPersonalData(data: UserPersonalDataDTO) {
  try {
    if (__DEV__) console.log('[USER_SERVICE] Salvando perfil completo via PUT (UPSERT)');
    ...
    if (__DEV__) console.log('[USER_SERVICE] Perfil completo salvo com sucesso');
    return response.data;
  } catch (error: any) {
    if (__DEV__) console.error('[USER_SERVICE] Erro ao salvar perfil completo:', error.message);
    if (__DEV__) console.error('[USER_SERVICE] Status:', error.response?.status);

    // Se PUT retornou 404, tentar POST como fallback (criar novo registro)
    if (error.response?.status === 404) {
      if (__DEV__) console.log('🔄 [USER_SERVICE] PUT retornou 404, tentando POST como fallback...');
      try {
        const postResponse = await api.post('user-personal-data', data, {
          timeout: 60000,
        });
        if (__DEV__) console.log('[USER_SERVICE] POST fallback bem sucedido');
        return postResponse.data;
      } catch (postError: any) {
        if (__DEV__) console.error('[USER_SERVICE] POST também falhou:', postError.message);
...
export async function saveUserPersonalData(data: UserPersonalDataDTO) {
  if (__DEV__) console.log('💾 [USER_SERVICE] Iniciando salvamento do perfil (apenas PUT)');

  try {
    // 1. Buscar dados existentes
    if (__DEV__) console.log('📥 [USER_SERVICE] Buscando dados existentes para merge...');
    ...
    if (__DEV__) console.log('[USER_SERVICE] Dados merged para salvamento');

    // 3. Salvar dados merged
    if (__DEV__) console.log('🔄 [USER_SERVICE] Salvando perfil completo (PUT)...');
    const result = await updateUserPersonalData(mergedData);
    if (__DEV__) console.log('✅ [USER_SERVICE] Perfil salvo com sucesso via PUT');
    return result;
  } catch (putError: any) {
    const errorMessage = putError.message?.toLowerCase() || '';
    const status = putError.response?.status;

    if (__DEV__) console.error('[USER_SERVICE] PUT/POST falhou:', putError.message, 'status:', status);
    ...
    if (
      ...
      if (__DEV__) console.log('⚠️ [USER_SERVICE] Erro "não encontrado" - pode ser bug do backend');
      if (__DEV__) console.log('🔄 [USER_SERVICE] Verificando se dados foram salvos...');
      ...
      try {
        ...
        if (checkData) {
          if (__DEV__) console.log('✅ [USER_SERVICE] Dados foram salvos com sucesso apesar do erro!');
          return checkData;
        }
      } catch (checkError) {
        if (__DEV__) console.log('❌ [USER_SERVICE] Dados não foram salvos');
      }
    }
...
export async function uploadProfilePhoto(imageUri: string): Promise<string> {
  try {
    if (__DEV__) console.log('📤 [USER_SERVICE] Iniciando upload de foto de perfil:', imageUri);
    ...
    if (__DEV__) console.log('📊 [USER_SERVICE] Tamanho da imagem base64:', Math.round(photoBase64.length / 1024), 'KB');
    ...
    if (__DEV__) console.log('✅ [USER_SERVICE] Foto de perfil enviada com sucesso');

    // Retornar a foto salva (pode ser processada pelo backend)
    return response.data.data?.photoBase64 || photoBase64;
  } catch (error: any) {
    if (__DEV__) console.error('[USER_SERVICE] Erro ao fazer upload da foto:', error.message);
    if (__DEV__) console.error('[USER_SERVICE] Status:', error.response?.status);
...
export async function deleteProfilePhoto(): Promise<void> {
  try {
    if (__DEV__) console.log('🗑️ [USER_SERVICE] Removendo foto de perfil...');

    await api.delete('user-personal-data/profile-photo');

    if (__DEV__) console.log('✅ [USER_SERVICE] Foto de perfil removida com sucesso');
  } catch (error: any) {
    if (__DEV__) console.error('❌ [USER_SERVICE] Erro ao remover foto de perfil:', error);
    ...
    if (error.response?.status === 400 && error.response?.data?.message?.includes('não possui foto')) {
      if (__DEV__) console.log('ℹ️ [USER_SERVICE] Usuário não tinha foto de perfil');
      return;
    }
...
export async function getNotificationPreferences(): Promise<NotificationPreferencesDTO> {
  try {
    if (__DEV__) console.log('📥 [USER_SERVICE] Buscando preferências de notificação...');

    const response = await api.get<{ success: boolean; data: NotificationPreferencesDTO }>(
      'user-personal-data/notification-preferences'
    );

    if (__DEV__) console.log('[USER_SERVICE] Preferências de notificação recuperadas');

    return response.data.data;
  } catch (error: any) {
    if (__DEV__) console.error('❌ [USER_SERVICE] Erro ao buscar preferências de notificação:', error);
    throw error;
  }
}
...
export async function updateNotificationPreferences(preferences: NotificationPreferencesDTO): Promise<void> {
  try {
    if (__DEV__) console.log('[USER_SERVICE] Atualizando preferências de notificação');

    await api.put('user-personal-data/notification-preferences', preferences);

    if (__DEV__) console.log('✅ [USER_SERVICE] Preferências de notificação atualizadas com sucesso');
  } catch (error: any) {
    if (__DEV__) console.error('❌ [USER_SERVICE] Erro ao atualizar preferências de notificação:', error);
    throw error;
  }
}
...
export async function getOnboardingStatus(): Promise<{ hasCompletedOnboarding: boolean }> {
  ...
  try {
    ...
    if (cachedStatus === 'true') {
      if (__DEV__) console.log('📱 [USER_SERVICE] Cache local indica onboarding completo - retornando TRUE');
      // Atualizar do backend em background (não bloqueia)
      updateOnboardingCacheFromBackend();
      return { hasCompletedOnboarding: true };
    }
    if (__DEV__) console.log('📱 [USER_SERVICE] Cache local não encontrado ou não é "true"');
  } catch (cacheError) {
    if (__DEV__) console.log('⚠️ [USER_SERVICE] Erro ao ler cache local:', cacheError);
  }

  // 2. Se não tem cache, buscar do backend
  try {
    if (__DEV__) console.log('📥 [USER_SERVICE] Verificando status do onboarding no backend...');
    ...
    if (__DEV__) console.log('✅ [USER_SERVICE] Status do onboarding do backend:', hasCompleted);

    // 3. Se o onboarding foi completado, salvar no cache local
    if (hasCompleted) {
      await AsyncStorage.setItem(ONBOARDING_CACHE_KEY, 'true');
      if (__DEV__) console.log('💾 [USER_SERVICE] Cache de onboarding salvo');
    }

    return { hasCompletedOnboarding: hasCompleted };
  } catch (error: any) {
    if (__DEV__) console.error('❌ [USER_SERVICE] Erro ao verificar status do onboarding:', error?.message);
    ...
    try {
      const cachedStatus = await AsyncStorage.getItem(ONBOARDING_CACHE_KEY);
      if (cachedStatus === 'true') {
        if (__DEV__) console.log('📱 [USER_SERVICE] Fallback: Cache local indica onboarding completo');
        return { hasCompletedOnboarding: true };
      }
      ...
      if (personalData) {
        ...
        if (parsed && (parsed.gender || parsed.weight || parsed.height)) {
          if (__DEV__) console.log('📱 [USER_SERVICE] Fallback: Dados pessoais encontrados, considerando onboarding completo');
          await AsyncStorage.setItem(ONBOARDING_CACHE_KEY, 'true');
          return { hasCompletedOnboarding: true };
        }
      }
    } catch (fallbackError) {
      if (__DEV__) console.error('❌ [USER_SERVICE] Erro no fallback:', fallbackError);
    }

    // Último recurso: sem cache e sem dados pessoais, assumir não completado
    if (__DEV__) console.log('⚠️ [USER_SERVICE] Nenhum indicador encontrado, onboarding será mostrado');
...
async function updateOnboardingCacheFromBackend(): Promise<void> {
  try {
    ...
    if (__DEV__) console.log('⚠️ [USER_SERVICE] Falha ao atualizar cache de onboarding (background)');
  }
}
...
export async function completeOnboarding(): Promise<void> {
  try {
    if (__DEV__) console.log('📤 [USER_SERVICE] Marcando onboarding como completo...');
    ...
    if (__DEV__) console.log('✅ [USER_SERVICE] Onboarding marcado como completo e salvo no cache local');
  } catch (error: any) {
    if (__DEV__) console.error('❌ [USER_SERVICE] Erro ao marcar onboarding como completo:', error);
    throw error;
  }
}
